...
class Creature:
    ...
    def exp(self):
        ...
        if self._exp == None:
            self._exp = 0
        #the higher the level the more exp for defeating (need refinement: level of opponent (other) should be looked at)
...
#--player goes first--- (higher speed stat)
        ...
        #-------------------------enemy turn--------------
            ...
            #compute and deal dmg of atks
                ...
                #---
                #add stats to dmg computation (if >= 0)
                if other._stats["atk"] - self._stats["dfn"] >= 0:
...
#--enemy goes first---(higher speed stat)
            #---------enemy turn---------------
        else:
